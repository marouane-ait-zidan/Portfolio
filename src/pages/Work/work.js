...
// Import local images
// import simpleStoreImg from '../assets/images/simple-store.png';
// import gestionDeStockImg from '../assets/images/gestion-de-stock.png';
...
function Work() {
    ...
    // Define the specific projects you want to display (by repository name)
    ...
    const descriptionMap = {
        ...
        'Simple-Store': 'A minimalist e-commerce platform built using React.js and Node.js, offering a fast and smooth shopping experience with clean design and performance-focused features.',
      };
    ...
    useEffect(() => {
        const fetchGitHubProjects = async () => {
            try {
                const response = await fetch('https://api.github.com/users/marouane-ait-zidan/repos');
                if (!response.ok) {
                    throw new Error(`GitHub API error: ${response.status} ${response.statusText}`);
                }
                const repos = await response.json();

                // Filter repos to only include the selected projects
                const filteredRepos = repos
                    .filter(repo => selectedProjects.includes(repo.name))
                    .sort((a, b) => selectedProjects.indexOf(a.name) - selectedProjects.indexOf(b.name))
                    .slice(0, 5);
    ...
    return (
        <div className="work">
            ...
            <div className="projects">
                ...
                {!loading && !error && projects.length === 0 && <p>No projects found. Check the console for more details.</p>}
